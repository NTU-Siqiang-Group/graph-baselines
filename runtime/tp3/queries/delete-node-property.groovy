...
v = g.V(vid).next().property(TEST_PROPERTY,TEST_PROPERTY);
if (!SKIP_COMMIT) {
    try {
        g.tx().commit();
    } catch (MissingMethodException e) {
...
execute_query(g,vid,SID,0,DATABASE,DATASET,QUERY,ITERATION,NODE_ARRAY,SID,TEST_PROPERTY,SKIP_COMMIT);
